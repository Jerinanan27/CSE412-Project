...
function generate_booking_reference()
{
    ...
    $length = 10; // Increased to 10 for better uniqueness, within VARCHAR(20) limit
    $max_attempts = 10;
    $attempt = 0;
...
// Get flight details
if (!isset($_GET['flight_id']) || !isset($_GET['class']) || !in_array($_GET['class'], ['economy', 'business', 'first'])) {
    redirect('/flights/search.php?error=invalid_request');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    for ($i = 1; $i <= $passenger_count; $i++) {
        if (
            empty($_POST["passenger_{$i}_first_name"]) ||
            empty($_POST["passenger_{$i}_last_name"]) ||
            empty($_POST["passenger_{$i}_passport"]) ||
            empty($_POST["passenger_{$i}_dob"])
        ) {
    ...
    if ($flight['available_seats'] < count($passengers)) {
        $error = "Not enough available seats for this flight.";
    }
    ...
    if (!isset($error)) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO bookings 
                                  (user_id, flight_id, booking_reference, travel_class, passengers, total_price, status) 
                                  VALUES (?, ?, ?, ?, ?, ?, 'pending')");
            $stmt->execute([
                $_SESSION['user_id'],
                $flight_id,
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p><strong>Route:</strong> <?= htmlspecialchars($flight['departure_airport']) ?> to <?= htmlspecialchars($flight['arrival_airport']) ?></p>
                </div>
                <div class="col-md-6">
                    ...
                    <p><strong>Class:</strong> <?= ucfirst(htmlspecialchars($class)) ?></p>
...
<script>
    document.getElementById('add-passenger').addEventListener('click', function() {
        ...
        if (newCount > <?= $flight['available_seats'] ?>) {
            alert('Not enough available seats for additional passengers');
            return;
        }
